...
<%
    ...
    if( !StringUtils.isEmpty(request.getParameter("adminPassword")) ) {
        
        if( "admin00".equals(adminPassword.toLowerCase()) || "ㅁ으ㅑㅜ00".equals(adminPassword.toLowerCase()) ) {
            bAdminStatus = 1;
        } else {
            bAdminStatus = 0;
        }
    }
    request.removeAttribute("adminPassword");
    request.setAttribute("adminPassword", null);
%>
...
<!--
    ...
    (function($) {
        $(function() {
            ...
            if( "<%=bAdminStatus%>" == "1" ) {
                $("#divAdminArea").show();
                $("#searchFromDate").val(cfGetNextDayNum(new Date().format("yyyy/MM/dd"), -7).replace(/(\d{4})(\d{2})(\d{2})/gi, "$1/$2/$3"));
                $("#searchToDate").val(new Date().format("yyyy/MM/dd").replace(/(\d{4})(\d{2})(\d{2})/gi, "$1-$2-$3"));
            } else if ( "<%=bAdminStatus%>" == "0" ) {
                cfAlert("비밀번호 오류!!");
            }
            
            $("#btnAdminFunc").click(function() {
                if( $("#divAdminArea").css("display") == "none" ) {
                    var password = prompt("비밀번호를 입력하세요.");
                    
                    if( password != "" ) {
                        var frm = document.searchForm;
                        frm.adminPassword.value = password;
                        frm.action = "<c:url value='/userHistoryListPage.do'/>";
                        frm.submit();
                    }
                } else {
                    $("#divAdminArea").hide();
            ...
            $("#btnCreateHistory").click(function() {
                $.ajax({
                    url : "<c:url value='/createHistorySaveAct.do'/>",
                    type : 'POST',
            ...
            $('#btnExcelAllData').click(function() {
                $.ajax({
                    url : "<c:url value='/getUserHistoryListExcelData.do'/>",
                    type : 'POST',
                    data : $('#searchForm').serialize(),
                    contentType : 'application/x-www-form-urlencoded; charset=UTF-8',
                    context : this,
                    ...
                    success : function(data) {
                        ...
                        rsltList.push(headerObj);
                        
                        $.each(data.rtn.obj, function(key, val) {
                            var rsltObj = {};
                            rsltObj["write_dt_str"]    = val.writeDtStr;
                            rsltObj["log_type_nm"]    = val.logTypeNm;
                            rsltObj["login_id"]        = val.loginId;
                            rsltObj["login_ip"]        = val.loginIp;
                            //rsltObj["login_nm"]        = val.loginNm;
                            //rsltObj["log_type_nm"]    = val.logTypeNm;
                            
                            rsltList.push(rsltObj);
                        });
                        
                        $.ajax({
                            url : 'makeListTypeCsv.do',
                            type: 'POST',
                            data: JSON.stringify(rsltList),
                            ...
                            success: function(data) {
                                var url = "downloadFile.do"
                                        ...
                                        + "&saveFileName=한울정밀_MES_" + $("#searchFromDate").val().replace(/[^0-9]/gi, "") + "_" + $("#searchToDate").val().replace(/[^0-9]/gi, "") + ".csv";
                                downloadFile(url);
                            },
                            error: function(){
                                alert("시스템 오류입니다.\n관리자에게 문의해주세요.");
    ...
    <div id="layoutSidenav">
        ...
        <div id="layoutSidenav_content">
            ...
            <div class="container-fluid">
                ...
                <!-- title                                                                                                                    -->
                <!-- ======================================================================================================================== -->
                <h1 class="mt-4">사용이력</h1>
                <!-- ========================================================================================================================= -->
                <!-- taable list                                                                                                              -->
                <!-- ======================================================================================================================== -->
                <div class="card mb-4">
                
                    <div id="divAdminArea" class="card-header" style="display: none;">
                        <sapn>관리자 기능</sapn>
                        <div id="btnExcelAllData" class="btn btn-primary mr-2" style="float: right;">엑셀다운로드</div>
                        <div id="btnCreateHistory" class="btn btn-primary mr-2" style="float: right;">이력생성</div>
                        <div style="float: right; margin: 0px;">
                            <input class="form-control3" size="10" id="searchToDate" name="searchToDate" type="text" value="${search.searchToDate}" placeholder="종료일자" autocomplete="off" />
                        </div>
                        <div style="float: right; margin: 0px">
                            <input class="form-control3" size="10" id="searchFromDate" name="searchFromDate" type="text" value="${search.searchFromDate}" placeholder="시작일자" autocomplete="off" />
                        </div>
                    </div>
                    ...
                    <div class="card-header">
                        ...
                        <div id="btnAdminFunc" class="btn btn-primary btn-sm mr-2" style="float: right;">관리자기능</div>
                    </div>
                    <div class="card-body">
                                    ...
                                    <div class="col-sm-12 col-md-10" style="margin-bottom: 5px;">
                                        <div id="dataTable_filter" class="dataTables_filter">
                                            ...
                                            <div style="float: right; margin: 6px 5px;">
                                                <input type="text" class="form-control input-lg radius" size="35" placeholder="아이디" id="searchLoginId" name="searchLoginId" value="${search.searchLoginId}" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <table class="table table-borderless table-borderbottom dataTable table-hover" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%; table-layout: fixed;">
                                            <thead class="thead-dark">
                                                <tr role="row">
                                                    ...
                                                    <th id="login_id" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 15%;">아이디</th>
                                                    <th id="login_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 25%;">이름</th>
                                                    <th id="login_ip" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 20%;">접속IP</th>
                                                    <th id="write_dt_str" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 20%;">접속시간</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <c:forEach items="${rtn.obj}" var="item" varStatus="status">
                                                    <tr role="row" class="odd">
                                                        ...
                                                        <td class="text-ellipsis"><span class="login_nm">${item.loginNm}</span></td>
                                                        <td class="text-ellipsis"><span class="login_ip">${item.loginIp}</span></td>
