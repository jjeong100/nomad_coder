...
<!--
	...
	(function($) {
				...
				buttons : [ {
					...
					click : function() {
						...
						cfAjaxCallAndGoLink("<c:url value='/materialInSaveAct.do'/>", $('#objForm').serialize()
								, '삭제 되었습니다.', 'searchForm', "<c:url value='/mobileMaterialInListPage.do'/>");
					}
				}, {
					text : "취소",
					click : function() {
			...
			$("#glv_upd_confirm").dialog({
				...
				buttons : [ {
					...
					click : function() {
						...
						cfAjaxCallAndGoLink("<c:url value='/materialInSaveAct.do'/>", $('#objForm').serialize()
								, '수정 되었습니다.', 'searchForm', "<c:url value='/mobileMaterialInListPage.do'/>");
					}
				}, {
					text : "취소",
					click : function() {
			...
			$("#glv_ins_confirm").dialog({
				...
				buttons : [ {
					...
					click : function() {
						...
						cfAjaxCallAndGoLink("<c:url value='/materialInSaveAct.do'/>", $('#objForm').serialize()
								, '생성 되었습니다.', 'searchForm', "<c:url value='/mobileMaterialInListPage.do'/>");
					}
				}, {
					text : "취소",
					click : function() {
		...
		if ($("#inCnt").val().trim() == '') {
			alert(MSG_COM_ERR_001.replace("[@]", " : 중량"));
			$("#inCnt").focus();
			return false;
		}

		if (!cfCheckDigit($("#inCnt").val())) {
			alert(MSG_COM_ERR_008.replace("[@]", " : 중량"));
			$("#inCnt").focus();
			return false;
		}
		return true;
...
<body>
							...
							<div class="form-row">
								<div class="col-md-4">
									<div class="form-group">
										...
										<input class="form-control3" id="inDt" name="inDt" type="text" value="${rtn.obj.inDt}" placeholder="입고일자를 입력하세요" />
									</div>
								</div>
								<div class="col-md-4">
									<div class="form-group">
								...
								<div class="col-md-4">
									<div class="form-group">
										<label class="small mb-1" for="inCnt" style="display: block;">중량(KG)</label>
										<input class="form-control" id="inCnt" name="inCnt" type="text" onkeypress="return isNumberKey(event)" value="${rtn.obj.inCnt}" placeholder="중량을 입력하세요" />
									</div>
								</div>
							</div>
							<div class="form-row">
								<div class="col-md-4">
									<div class="form-group">
										...
										<select class="selectpicker" id="matCd" name="matCd" data-size="7" data-live-search="true">
											<option value="">자재 선택</option>
											<c:forEach items="${material_list}" var="item" varStatus="status">
												<c:choose>
													<c:when test="${item.matCd == rtn.obj.matCd}">
														<option value="${item.matCd}" data-custCd="${item.matCustCd}" data-custNm="${item.matCustNm}" data-matItemNm="${item.matItemNm}" selected>${item.matNm}</option>
													</c:when>
													<c:otherwise>
														<option value="${item.matCd}" data-custCd="${item.matCustCd}" data-custNm="${item.matCustNm}" data-matItemNm="${item.matItemNm}">${item.matNm}</option>
													</c:otherwise>
												</c:choose>
											</c:forEach>
										</select>
                                 ...
                                 <div class="col-md-4">
									<div class="form-group">
										...
										<input class="form-control" id="custCd" name="custCd" type="text" value="${rtn.obj.custNm}" data-key="${rtn.obj.custCd}" readonly/>
									</div>
								</div>
								<div class="col-md-4">
									<div class="form-group">
										<label class="small mb-1" for="matItemNm" style="display: block;">품명</label>
										<input class="form-control" id="matItemNm" name="matItemNm" type="text"  value="${rtn.obj.matItemNm}"readonly/>
									</div>
								</div>
							</div>
							<div class="form-row">
	...
	<form:form commandName="search" id="searchForm" name="searchForm">
		...
		<input type="hidden" name="searchCustCd" value="${search.searchCustCd}" />
