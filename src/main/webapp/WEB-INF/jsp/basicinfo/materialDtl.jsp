...
<!--
    ...
    (function($) {
        $(function() {
            ...
            $('#btnUpdateClose,#btnCreateClose').click(function() {
                ...
                frm.action = "<c:url value='/materialListPage.do'/>";
                frm.submit();
            });
            ...
            $("#glv_del_confirm").dialog({
                ...
                buttons : [ {
                    ...
                    click : function() {
                        ...
                        cfAjaxCallAndGoLink("<c:url value='/materialSaveAct.do'/>", $('#objForm').serialize(), '삭제 되었습니다.', 'searchForm', "<c:url value='/materialListPage.do'/>");
                    }
                }, {
                    text : "취소",
                    click : function() {
            ...
            $("#glv_upd_confirm").dialog({
                ...
                buttons : [ {
                    ...
                    click : function() {
                        ...
                        cfAjaxCallAndGoLink("<c:url value='/materialSaveAct.do'/>", $('#objForm').serialize(), '수정 되었습니다.', 'searchForm', "<c:url value='/materialListPage.do'/>");
                    }
                }, {
                    text : "취소",
                    click : function() {
            ...
            $("#matCustCd,#matItemNm,#matSize").change(function(event) {
                var matCustCd=$('#matCustCd option:selected').attr("data-key");
                var matItemNm=$('#matItemNm').val();
                var matSize=$('#matSize').val();
                if (matCustCd==null) {
                    matCustCd='';
                }
                if ($("#matItemNm").val().trim() != '') {
                    if ( matCustCd!= '') {
                        matCustCd=matCustCd+'_'+matItemNm;
                    }
                    else{
                        matCustCd=matCustCd+matItemNm;
                    }
                }
                if ($("#matSize").val().trim() != '') {
                    if ( matCustCd!= '') {
                        matCustCd=matCustCd+'_'+matSize;
                    }
                    else{
                        matCustCd=matCustCd+matSize;
                    }
                }
                $("#matNm").val(matCustCd);
            });
            ...
            $("#glv_ins_confirm").dialog({
                ...
                buttons : [ {
                    ...
                    click : function() {
                        ...
                        cfAjaxCallAndGoLink("<c:url value='/materialSaveAct.do'/>", $('#objForm').serialize(), '생성 되었습니다.', 'searchForm', "<c:url value='/materialListPage.do'/>");
                    }
                }, {
                    text : "취소",
                    click : function() {
    ...
    function fn_submit_validation() {
        if ($("#matCustCd").val().trim() == '') {
            alert(MSG_COM_ERR_001.replace("[@]", " : 자재처"));
            $("#matCustCd").focus();
            return false;
        }
        if ($("#matItemNm").val().trim() == '') {
        ...
        if ($("#matSize").val().trim() == '') {
            alert(MSG_COM_ERR_001.replace("[@]", " : 규격"));
            $("#matSize").focus();
            return false;
            }
    ...
    function fn_btn_show_hide() {
        if (document.objForm.crudType.value == 'R') {
            ...
            //$('.selectpicker').next("button").css({"background-color":"rgb(233, 236, 239)", "border-color":"#ced4da"});

            // 입고처리된 qm건 수정,삭제 불가 처리
            if( ${rtn.obj.bomCnt > 0 || rtn.obj.matInCnt > 0 } ) {
                $("#btnDelete").addClass("disabled");

                $("#btnDelete").unbind().click(function(){
                    cfAlert("자재입고가 되거나 BOM 자재 소요정보에 등록되어져 있는 자재는 삭제할 수 없습니다.");
                    return false;
                });
            }
            ...
            $('#viewUpdateEnable').hide();
            $('#viewCreate').show();
            //$('#matTypeCd').val('MT').prop("selected",true);
        }
        $("#matCd").attr("readOnly", true);
        $('#matCd').css("background-color", "#e9ecef");
    }
    -->
</script>
</head>
...
<body>
                    ...
                    <div class="card mb-4" style="margin-top: 30px;">
                        ...
                        <div class="card-body">
                            <div class="form-row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label class="small mb-1" for="matNm">자재명</label>
                                        <input class="form-control" id="matCd" name="matCd" type="hidden" value="${rtn.obj.matCd}" placeholder="자재코드는 자동생성됩니다.">
                                        <input class="form-control" id="matNm" name="matNm" type="text" value="${rtn.obj.matNm}" readonly placeholder="자재명은 자재처+품명+규격 입니다.">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label class="small mb-1" for="matCustCd">자재처 </label>
                                        <select class="form-control2" id="matCustCd" name="matCustCd">
                                            <option value="">자재처 선택</option>
                                            <c:forEach items="${mat_cust_list}" var="item" varStatus="status">
                                                <c:choose>
                                                    <c:when test="${item.custCd == rtn.obj.matCustCd}">
                                                        <option value="${item.custCd}" data-key="${item.custNm}" selected>${item.custNm}</option>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <option value="${item.custCd}" data-key="${item.custNm}">${item.custNm}</option>
                                                    </c:otherwise>
                                                </c:choose>
                                            </c:forEach>
                                        </select>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label class="small mb-1" for="matItemNm">품명</label>
                                        <input class="form-control" id="matItemNm" name="matItemNm" type="text" value="${rtn.obj.matItemNm }"  placeholder="품명을 입력하세요." />
                                    </div>
                                </div>
                            ...
                            <div class="form-row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="small mb-1" for="matSize">자재 규격</label>
                                        <input class="form-control" id="matSize" name="matSize" type="text" onkeypress="return isNumberKey(event)"value="${rtn.obj.matSize}" placeholder="규격을 입력하세요">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
    ...
    <form:form commandName="search" id="searchForm" name="searchForm">
        ...
        <input type="hidden" name="searchMatCustNm" value="${search.searchMatCustNm}" />
        <input type="hidden" name="searchMatNm" value="${search.searchMatNm}" />
    </form:form>
    <script>
