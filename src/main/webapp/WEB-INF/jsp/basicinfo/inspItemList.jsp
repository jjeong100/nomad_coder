...
<head>
    ...
    <script type="text/javaScript" language="javascript" defer="defer">
        ...
        (function($) {
            $(function(){
                ...
                $('#btnCreate').click(function() {
                    ...
                    frm.action = "<c:url value='/inspItemDtlPage.do'/>";
                    frm.submit();
                });
        ...
        //=====================================================================
        function fn_go_dtl_page(type, cd) {
            var frm = document.searchForm;
            frm.crudType.value = 'R';
            frm.inspTypeCd.value = type;
            frm.inspItemCd.value = cd;
            frm.action = "<c:url value='/inspItemDtlPage.do'/>";
            frm.submit();
        }
		        ...
		        <div class="container-fluid">
                   ...
                   <div class="card mb-4">
                       <div class="card-header">
                           ...
                           <div id="btnCreate" class="btn btn-primary btn-sm mr-2" style="float: right;">검사항목 추가</div>
                       </div>
                       <div class="card-body">
                                       ...
                                       <div class="col-sm-12 col-md-10" style="margin-bottom:5px;">
                                           <div id="dataTable_filter" class="dataTables_filter">
                                               <form:form commandName="search" id="searchForm" name="searchForm">
                                                   ...
                                                   <input type="hidden" name="inspTypeCd" value="${search.inspTypeCd}"/>
                                                   <input type="hidden" name="inspItemCd" value="${search.inspItemCd}"/>
								                   ...
								                   <div style="float: right; margin: 6px 5px; width:200px">
								                       <select class="form-control2" id="searchInspTypeCd" name="searchInspTypeCd">
                                                           <option value="">검사타입 선택</option>
                                                           <c:forEach items="${insp_type_cd_list}" var="item" varStatus="status">
				                                               <c:choose>
				                                                   <c:when test="${item.scode == search.searchInspTypeCd}">
								                   ...
								                   <div style="float: right; margin: 6px 5px;">
								                       <input class="form-control2" id="searchInspItemNm" name="searchInspItemNm" type="text" value="${search.searchInspItemNm}" placeholder="검사항목명"/>
								                   </div>
                                           ...
                                           <table class="table table-borderless table-borderbottom dataTable table-hover" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%; table-layout:fixed;">
                                              <thead class="thead-dark">
                                                  <tr role="row">
	                                                  ...
	                                                  <th id="insp_type_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 20%;">검사타입구분</th>
	                                                  <th id="insp_item_cd" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 20%;">검사항목코드</th>
	                                                  <th id="insp_item_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 30%;">검사항목명</th>
	                                                  <th id="insp_item_type_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 20%;">검사방법구분</th>
	                                                  <th id="high_val" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">상한값</th>
	                                                  <th id="low_val" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">하한값</th>
	                                                  <th id="insp_danwi_cd" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">단위</th>
                                                  </tr>
                                              </thead>
                                              ...
                                              <tbody>
	                                              <c:forEach items ="${rtn.obj}" var="item" varStatus="status">
					                                <tr role="row" class="odd">
					                                    ...
					                                    <td class="text-ellipsis"><span class="insp_type_nm">${item.inspTypeNm}</span></td>
					                                    <td class="text-ellipsis"><span class="insp_item_cd">${item.inspItemCd}</span></td>
					                                    <td class="text-ellipsis"><span class="insp_item_nm"><a href="javascript:fn_go_dtl_page('${item.inspTypeCd}','${item.inspItemCd}')">${item.inspItemNm}</a></span></td>
					                                    <td class="text-ellipsis"><span class="insp_item_type_nm">${item.inspItemTypeNm}</span></td>
					                                    <td class="text-ellipsis"><span class="high_val">${item.highVal}</span></td>
					                                    <td class="text-ellipsis"><span class="low_val">${item.lowVal}</span></td>
					                                    <td class="text-ellipsis"><span class="insp_danwi_cd">${item.inspDanwiCd}</span></td>
					                                </tr>
					                              </c:forEach>
					                              <c:if test="${fn:length(rtn.obj) le 0}">
					                                <tr>
					                                    <td colspan="58 class="no-data">- 표시할 내용이 없습니다. -</td>
					                                </tr>
					                              </c:if>
                                              </tbody>
