    ...
    <script type="text/javaScript" language="javascript" defer="defer">
        ...
        (function($) {
            $(function(){
                ...
                $('span[name="btnEquipMtnCreate"]').click(function() {
                	var frm = document.equipMtnForm;
                	frm.equipSeq.value = $(this).attr("data-key");
                    frm.crudType.value = 'C';
                    frm.action = "<c:url value='/equipMtnDtlPage.do'/>";
                    frm.submit();
                });

                $('span[name="btnEquipMtnView"]').click(function() {
                	drawEquipMinList($(this).attr("data-key"));
                });
                drawEquipMinList = function(equipSeq) {
                    document.equipMtnForm.searchEquipSeq.value = equipSeq;
                    $.ajax({
                        url : "<c:url value='/getEquipMtnListData.do'/>",
                        type: 'POST',
                        data: $('#equipMtnForm').serialize(),
                        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                        context:this,
                        dataType:'json',
                        beforeSend:function(){

                            $("#equipMtnListBody tr").remove();
                        ...
                        success: function(data) {
                            ...
                            $.each(data.rtn.obj, function(key, val){
                                ...
                                html += "<td class=\"text-ellipsis\" style=\"width:5%\"><span>"+val.rnum+"</span></td>";
                                html += "<td class=\"text-ellipsis \" style=\"width:10%\"><span><a href=\"javascript:fn_go_minDtl_page('" + val.equipSeq + "', '" + val.equipMtnSeq + "')\">"+val.equipChkDt+"</a></span></td>";
                                html += "<td class=\"text-ellipsis \" style=\"width:10%\"><span class=\"text-ellipsis\" style=\"width:80px;display:inline-block;\">"+val.equipChkCont+"</span></td>";
                                html += "</tr>";

                                $("#equipMtnListBody").append(html);
                            });
                        },
                ...
                $('#btnCreate').click(function() {
                	var frm = document.searchForm;
                    frm.crudType.value = 'C';
                    frm.action = "<c:url value='/equipDtlPage.do'/>";
                    frm.submit();
                });
        ...
        function fn_go_dtl_page(id) {
            ...
            frm.equipSeq.value = id;
            frm.action = "<c:url value='/equipDtlPage.do'/>";
            frm.submit();
        }
        function fn_go_minDtl_page(equipSeq, equipMtnSeq) {
            var frm = document.equipMtnForm;
        	frm.equipSeq.value = equipSeq;
        	frm.equipMtnSeq.value = equipMtnSeq;
            frm.crudType.value = 'R';
            frm.action = "<c:url value='/equipMtnDtlPage.do'/>";
            frm.submit();
        }
...
<body>
		        ...
		        <div class="container-fluid">
                   ...
                   <div class="card mb-4">
                       <div class="card-header">
                           ...
                           <div id="btnCreate" class="btn btn-primary btn-sm mr-2" style="float: right;">설비 추가</div>
                       </div>
                       <div class="card-body">
                                       ...
                                       <div class="col-sm-12 col-md-10" style="margin-bottom:5px;">
                                           <div id="dataTable_filter" class="dataTables_filter">
                                               <form:form commandName="search" id="searchForm" name="searchForm">
                                                   ...
                                                   <input type="hidden" name="equipSeq" value="${search.equipSeq}"/>
								                   ...
								                   <div style="float: right; margin: 6px 5px;">
								                   		<input type="text" class="form-control col-xs-2 input-lg radius" size="35" placeholder="설비명" id="searchEquipNm" name="searchEquipNm" value="${search.searchEquipNm}" />
								                   </div>
                                           ...
                                           <table class="table table-borderless table-borderbottom dataTable table-hover" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%; table-layout:fixed;">
                                              <thead class="thead-dark">
                                                  <tr role="row">
	                                                  ...
	                                                  <th id="equip_model_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 12%;">모델명</th>
	                                                  <th id="equip_corp_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">제작사</th>
	                                                  <th id="equip_get_dt" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">구입일자</th>
	                                                  <th id="" class="no-sort" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">유지보수내역</th>
                                                  </tr>
                                              </thead>
                                              ...
                                              <tbody>
	                                              <c:forEach items ="${rtn.obj}" var="item" varStatus="status">
					                                <tr role="row" class="odd">
														...
														<td class="text-ellipsis"><span class="equip_nm"><a href="javascript:fn_go_dtl_page('${item.equipSeq}')">${item.equipNm}</a></span></td>
														<td class="text-ellipsis"><span class="equip_model_nm">${item.equipModelNm}</span></td>
														<td class="text-ellipsis"><span class="equip_corp_nm">${item.equipCorpNm}</span></td>
														<td class="text-ellipsis"><span class="equip_get_dt">
															<fmt:parseDate value="${item.equipGetDt}" var="equipGetDt" pattern="yyyyMMdd"/>
															<fmt:formatDate pattern="yyyy/MM/dd" value="${equipGetDt}"/></span>
														</td>
														<td class="text-ellipsis">
														<span name="btnEquipMtnCreate" class="btn btn-primary btn-sm" data-key="${item.equipSeq}">등록</span>
														<span name="btnEquipMtnView" class="btn btn-primary btn-sm" data-key="${item.equipSeq}">조회</span>
														</td>
					                                </tr>
					                              </c:forEach>
                                           ...
                                           <table class="table table-borderless table-borderbottom dataTable table-hover" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%; table-layout:fixed;">
                                              <thead class="thead-dark">
                                                  <tr role="row">
                                                      <!-- th id는 소트 쿼리에 그대로 사용됨, 실체 컬럼명과 일치 해야함. -->
	                                                  <th id="rnum" class="no-sort" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 5%;">No</th>
	                                                  <th id="equip_get_dt" class="no-sort" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">구입일자</th>
	                                                  <th id="equip_chk_cont" class="no-sort" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">유지보수내역</th>
                                                  </tr>
                                              </thead>
                                              <tbody id="equipMtnListBody">
                                              </tbody>
    ...
    <form:form commandName="search" id="equipMtnForm" name="equipMtnForm">
    <input type="hidden" name="crudType" value=""/>
    <input type="hidden" name="searchEquipSeq" value=""/>
    <input type="hidden" name="equipSeq" value=""/>
    <input type="hidden" name="equipMtnSeq" value=""/>
</form:form>
<script>
