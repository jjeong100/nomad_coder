    ...
    <script type="text/javaScript" language="javascript" defer="defer"> 
        ...
        (function($) {
            $(function(){
                ...
                drawChart = function() {
                    $.ajax({
                        url : "<c:url value='/searchDeliveryStaticsListData.do'/>",
                        type: 'POST',
...
//                                      }
                ...
                drawTot = function() {
                    $.ajax({
                        url : "<c:url value='/selectDeliveryTotStaticsData.do'/>",
                        type: 'POST',
                        data: $('#searchForm').serialize(),
                        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                        context:this,
                        ...
                        success: function(data) {
                            $("#layoutSidenav").loadingClose();
                            if (data.rtn.rc == 0) {
                                $("#titleMsg").html('납기단축일&nbsp;&nbsp;&nbsp;&nbsp;기준년도&nbsp;:&nbsp;'+data.rtn.obj.baseYearTotRate+'&nbsp;&nbsp;&nbsp;&nbsp;대상년도&nbsp;:&nbsp;'+data.rtn.obj.compYearTotRate);
                            }
                        },
                        error: function(request, status, error){
                            console.log("code = "+ request.status + " message = " + request.responseText + " error = " + error);
