...
<!--
	...
	(function($) {
		$(function() {
			...
			$('#btnSearch').click(function() {
				...
				frm.action = "<c:url value='/itemOutSetPage.do'/>";
				frm.submit();
			});
			...
			// 바코드 출력 팝업
			//-------------------------------------------------------------
			$("#btnOutSet").click(function() {
				var outSetCnt = $("input:radio[name='rowCheckBox']:checked").length;

				if (outSetCnt == 0) {
					cfAlert("출고 처리할 상품을 선택하세요.");
					return false;
				}

				if (outSetCnt > 10) {
					cfAlert("출고처리는 10개까지만 가능합니다.");
					return false;
				}

				$("input[name=rowCheckBox]:checked").each(function(idx, obj) {
					fn_itemOutSetList($(this).val());
				});
			});
            ...
            $(document).scannerDetection({
                ...
                onComplete: function(barcode, qty){
                	...
                	fn_itemOutSetList(barcode);
                } // main callback function
            });
            $( "#glv_outset_confirm" ).dialog({
                autoOpen: false,
                width: 400,
                buttons: [
                    {
                        ...
                        click: function() {
                            $( this ).dialog( "close" );
                            cfAjaxCallAndGoLink("<c:url value='/itemOutSetSave.do'/>"
                                    ,$('#itemOutForm').serialize(),'출고처리 되었습니다.'
                                    ,'searchForm'
                                    ,"<c:url value='/itemOutSetPage.do'/>");
                        }
                    },
                    {
                        text: "취소",
	...
	function fn_itemOutSetList(lotid) {
		 var frm = document.itemOutForm;
		 frm.lotOutId.value=lotid;
		 $( "#glv_outset_confirm" ).dialog( "open" );
	}
	-->
</script>
</head>
...
<body>
			...
			<div class="container-fluid">
				...
				<div class="card mb-4">
					<div class="card-header">
						<sapn>출하 대상 목록</sapn>
						<div id="btnOutSet" class="btn btn-primary btn-sm mr-2" style="float: right;">출고처리</div>
					</div>
					<div class="card-body">
											...
											<table class="table table-borderless table-borderbottom dataTable table-hover" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;  table-layout: fixed;">
												<thead class="thead-dark">
													<tr role="row">
	                                                    ...
	                                                    <th id="itemout_dt" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">출고일자</th>
	                                                    <th id="item_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 15%;">제품명</th>
	                                                    <th id="out_qty" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 8%;">출고수량</th>
	                                                    <th id="item_out_type_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 8%;">출고구분</th>
	                                                    <th id="cust_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 8%;">납품처</th>
	                                                    <th id="lotid" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">입고LOT</th>
	                                                    <th id="lot_out_id" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">출고LOT</th>
													</tr>
												</thead>
												<tbody>
													<c:forEach items="${rtn.obj}" var="item" varStatus="status">
														<tr role="row" class="odd">
															<c:if test="${item.itemOutTypeCd =='OUT_WAIT'}">
															<td class="text-ellipsis" style="width:8%;">
															<input type="radio" name="rowCheckBox" value="${item.lotOutId}"/>
															<div id="${item.lotOutId}_itemNm" style="display:none;">${item.itemNm}</div>
															<div id="${item.lotOutId}_custNm" style="display:none;">${item.custNm}</div>
							                            	<div id="${item.lotOutId}_outQty" style="display:none;">cfAddComma(cfIsNullString(${item.outQty},"0"))</div>
							                            	<div id="${item.lotOutId}_itemoutDt" style="display:none;">${item.itemoutDt}</div>
														    </td>
														    </c:if>
														    <c:if test="${item.itemOutTypeCd !='OUT_WAIT'}">
															<td class="text-ellipsis" style="width:8%;">
															...
															<td class="text-ellipsis"><span class="itemout_dt">
															<fmt:parseDate value="${item.itemoutDt}" var="itemoutDt" pattern="yyyyMMdd"/>
			                                                  <fmt:formatDate pattern="yyyy/MM/dd" value="${itemoutDt}"/></span>
															</td>
															<td class="text-ellipsis"><span class="item_nm">${item.itemNm}</span></td>
															<td class="text-ellipsis"><span class="out_qty"><fmt:formatNumber value="${item.outQty}" pattern="###,###"/>개</span></td>
															<td class="text-ellipsis"><span class="item_out_type_nm">${item.itemOutTypeNm}</span></td>
															<td class="text-ellipsis"><span class="cust_nm">${item.custNm}</span></td>
															<td class="text-ellipsis"><span class="lotid">${item.lotid}</td>
															<td class="text-ellipsis"><span class="lot_out_id">${item.lotOutId}</td>
														</tr>
													</c:forEach>
	...
	<form:form commandName="search" id="itemOutForm" name="itemOutForm">
		<input type="hidden" name="lotOutId" value="" />
	</form:form>
	<div id="glv_outset_confirm" title="확인" style="display:none;">
