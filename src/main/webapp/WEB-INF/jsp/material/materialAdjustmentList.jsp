...
<head>
    ...
    <script type="text/javaScript" language="javascript" defer="defer"> 
        ...
        (function($) {
            $(function(){
                ...
                $('#btnSearch').click(function() {
                    ...
                    frm.action = "<c:url value='/materialAdjustmentListPage.do'/>";
                    frm.submit();
                });
        ...
        // 수정
        //===================================================================== 
        function fn_update(matinSeq,matoutSeq,idx) {
        	var inCnt  = fn_undefined($("#inCnt_"+idx).val());
        	var outCnt = fn_undefined($("#outCnt_"+idx).val());
            $("#glv_upd_confirm")
            .data("matinSeq",matinSeq)
            .data("inCnt",inCnt)
            .data("matoutSeq",matoutSeq)
            .data("outCnt",outCnt)
            .dialog("open");
        }
...
<!--                            <div id="btnClose" class="btn btn-primary btn-sm mr-2" style="float: right;">자재 조정 처리</div> -->
<!--                            <div id="btnUnClose" class="btn btn-primary btn-sm mr-2" style="float: right;">자재 조정</div> -->
                       </div>
                       <div class="card-body">
                                           ...
                                           <table class="table table-borderless table-borderbottom dataTable table-hover" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%; table-layout:fixed;">
                                              <thead class="thead-dark">
                                                  <tr role="row">
                                                      ...
                                                      <th id="workshop_nm" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">창고명</th>
                                                      <th id="lotid"       class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">Lot No.</th>
                                                      <th id="prod_po_no"  class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">작업지시</th>
                                                      <th id="item_nm"     class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 20%;">제품명</th>
                                                      <th id="oper_nm"     class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 15%;">공정명</th>
                                                      <th id="in_cnt"      class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">입고량</th>
                                                      <th id="out_cnt"     class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">출고량</th>
                                                      <th id="btn_update"  class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 10%;">수정</th>
                                                  </tr>
                                              </thead>
                                              <tbody>
                                                  <c:forEach items ="${rtn.obj}" var="item" varStatus="status">
                                                    <tr role="row" class="odd">
                                                        ...
                                                        <td class="text-ellipsis"><span class="in_cnt"><input class="form-control col-xs-2 input-lg radius" id="inCnt_${status.count}" name="inCnt" type="text" value="<fmt:formatNumber value="${item.inCnt}" pattern="###,###"/>"/></span></td>
                                                        
                                                        <c:choose>
                                                            <c:when test="${(item.matoutSeq ne null)}">
                                                                <td class="text-ellipsis"><span class="out_cnt"><input class="form-control col-xs-2 input-lg radius" id="outCnt_${status.count}" name="outCnt" type="text" value="<fmt:formatNumber value="${item.outCnt}" pattern="###,###"/>"/></span></td>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <td class="text-ellipsis"><span class="out_cnt"><input class="form-control col-xs-2 input-lg radius" id="outCnt_${status.count}" name="outCnt" type="text" value="<fmt:formatNumber value="${item.outCnt}" pattern="###,###"/>" readonly/></span></td>
                                                            </c:otherwise>
                                                        </c:choose>
                                                        
                                                        <td class="text-ellipsis"><span name="btnUpdate" class="btn btn-primary btn-sm" title="수정"  onclick="javaScript:fn_update('${item.matinSeq}','${item.matoutSeq}','${status.count}');">수정</span></td>
                                                    </tr>
                                                  </c:forEach>
