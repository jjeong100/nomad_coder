...
@Repository("bomInspDao")
public class BomInspDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mcc050";
	...
	public RtnVO select(BomInspVO param) {
		RtnVO rtn = new RtnVO();
		try {
			BomInspVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(BomInspVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<BomInspVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
	...
	public RtnVO selectInspList(BomInspVO param) {
        RtnVO rtn = new RtnVO();
        try {
            List<BomInspVO> list = sqlSession.selectList(ENTITY_NAME+".selectInspList", param);
            rtn.setObj(list);
        } catch (Exception e) {
            return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
        }
