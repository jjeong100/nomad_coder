...
public class DivisionDao extends BaseDao {
	...
	public final String ENTITY_NAME = "mcc005";
	...
	public RtnVO select(DivisionVO param) {
		RtnVO rtn = new RtnVO();
		try {
			DivisionVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(DivisionVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<DivisionVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
