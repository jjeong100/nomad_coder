...
public class CodeDao extends BaseDao {
	...
	public final String ENTITY_NAME = "mcc001";
	...
	public RtnVO select(CodeVO param) {
	    RtnVO rtn = new RtnVO();
	    try {
	        CodeVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
	        rtn.setObj(vo);
	    } catch (Exception e) {
	...
	public RtnVO searchList(CodeVO param) {
        RtnVO rtn = new RtnVO();
        try {
            List<CodeVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
            rtn.setObj(list);
        } catch (Exception e) {
            return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
        }
