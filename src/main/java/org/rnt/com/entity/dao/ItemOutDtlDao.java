...
@Repository("itemOutDtlDao")
public class ItemOutDtlDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mwp016";
	...
	public RtnVO select(ItemOutDtlVO param) {
		RtnVO rtn = new RtnVO();
		try {
			ItemOutDtlVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(ItemOutDtlVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<ItemOutDtlVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
