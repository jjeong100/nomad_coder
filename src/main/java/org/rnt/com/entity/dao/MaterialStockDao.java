...
public class MaterialStockDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mwp001";
	...
	public RtnVO select(MaterialStockVO param) {
		RtnVO rtn = new RtnVO();
		try {
			MaterialStockVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(MaterialStockVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<MaterialStockVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
