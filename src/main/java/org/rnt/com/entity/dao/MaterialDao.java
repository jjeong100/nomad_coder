...
public class MaterialDao extends BaseDao {
	...
	public final String ENTITY_NAME = "mcc010";
	...
	public RtnVO select(MaterialVO param) {
		RtnVO rtn = new RtnVO();
		try {
			MaterialVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(MaterialVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<MaterialVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
	...
	public RtnVO searchPopBomMatList(MaterialVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<MaterialVO> list = sqlSession.selectList(ENTITY_NAME+".searchPopBomMatList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
