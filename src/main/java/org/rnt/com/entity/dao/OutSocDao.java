...
@Repository("outSocDao")
public class OutSocDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mma020";
	...
	public RtnVO select(OutSocVO param) {
		RtnVO rtn = new RtnVO();
		try {
			OutSocVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(OutSocVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<OutSocVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
