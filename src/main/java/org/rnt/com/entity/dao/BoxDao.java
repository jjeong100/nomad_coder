...
public class BoxDao extends BaseDao {
	...
	public final String ENTITY_NAME = "mcc017";
	...
	public RtnVO select(BoxVO param) {
		RtnVO rtn = new RtnVO();
		try {
			BoxVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(BoxVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<BoxVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
