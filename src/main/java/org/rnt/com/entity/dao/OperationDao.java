...
public class OperationDao extends BaseDao {
	...
	public final String ENTITY_NAME = "mcc030";
	...
	public RtnVO select(OperationVO param) {
		RtnVO rtn = new RtnVO();
		try {
			OperationVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(OperationVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<OperationVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
