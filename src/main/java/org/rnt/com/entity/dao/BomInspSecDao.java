...
@Repository("bomInspSecDao")
public class BomInspSecDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mcc051";
	...
	public RtnVO select(BomInspSecVO param) {
		RtnVO rtn = new RtnVO();
		try {
			BomInspSecVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(BomInspSecVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<BomInspSecVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
