...
@Repository("calanderDao")
public class CalanderDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mcc009";
	...
	public RtnVO select(CalanderVO param) {
		RtnVO rtn = new RtnVO();
		try {
			CalanderVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(CalanderVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<CalanderVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
