...
@Repository("bomHistoryDao")
public class BomHistoryDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mcc021";
	...
	public RtnVO select(BomHistoryVO param) {
		RtnVO rtn = new RtnVO();
		try {
			BomHistoryVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(BomHistoryVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<BomHistoryVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
