...
public class DocumentDao extends BaseDao {
	...
	public final String ENTITY_NAME = "mcc906";
	...
	public RtnVO select(DocumentVO param) {
		RtnVO rtn = new RtnVO();
		try {
			DocumentVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(DocumentVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<DocumentVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
