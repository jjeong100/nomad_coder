...
@Repository("finishQmBadDao")
public class FinishQmBadDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mqc002";
	...
	public RtnVO select(FinishQmBadVO param) {
		RtnVO rtn = new RtnVO();
		try {
			FinishQmBadVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO selectList(FinishQmBadVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<FinishQmBadVO> list = sqlSession.selectList(ENTITY_NAME+".selectList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
	...
	public RtnVO updateActbadQty(FinishQmBadVO param) {
		RtnVO rtn = new RtnVO();
		try {
			sqlSession.update(ENTITY_NAME+".updateActbadQty", param);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_UPD_ERR_MSG, e.getMessage()); 
		}
		return rtn;
