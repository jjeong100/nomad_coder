...
@Repository("itemGrpMainDao")
public class ItemGrpMainDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mcc014";
	...
	public RtnVO select(ItemGrpMainVO param) {
		RtnVO rtn = new RtnVO();
		try {
			ItemGrpMainVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(ItemGrpMainVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<ItemGrpMainVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
