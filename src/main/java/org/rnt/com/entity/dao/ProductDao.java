...
public class ProductDao extends BaseDao {
	...
	public final String ENTITY_NAME = "mcc016";
	...
	public RtnVO select(ProductVO param) {
		RtnVO rtn = new RtnVO();
		try {
			ProductVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO delete_update(ProductVO param) {
		RtnVO rtn = new RtnVO();
		try {
			sqlSession.update(ENTITY_NAME+".delete_update", param);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_UPD_ERR_MSG, e.getMessage()); 
		}
		return rtn;
	...
	public RtnVO delete(ProductVO param) {
		RtnVO rtn = new RtnVO();
		try {
			sqlSession.delete(ENTITY_NAME+".delete", param);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_UPD_ERR_MSG, e.getMessage()); 
		}
		return rtn;
	...
	public RtnVO searchList(ProductVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<ProductVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
