...
@Repository("dataBackupDao")
public class DataBackupDao extends BaseDao {

	@Autowired
	private SqlSession sqlSession;

	public final String ENTITY_NAME = "mcc915";
	...
	public RtnVO select(DataBackupVO param) {
		RtnVO rtn = new RtnVO();
		try {
			DataBackupVO vo = sqlSession.selectOne(ENTITY_NAME+".select", param);
			rtn.setObj(vo);
		} catch (Exception e) {
	...
	public RtnVO searchList(DataBackupVO param) {
		RtnVO rtn = new RtnVO();
		try {
			List<DataBackupVO> list = sqlSession.selectList(ENTITY_NAME+".searchList", param);
			rtn.setObj(list);
		} catch (Exception e) {
			return setRtnVO(rtn, ENTITY_NAME+GlvConst.STR_SPACE+GlvConst.RC_DB_SEL_ERR_MSG, e.getMessage()); 
		}
