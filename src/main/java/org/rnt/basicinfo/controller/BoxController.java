...
public class BoxController extends BaseController {
    ...
    @RequestMapping(value = "/boxListPage.do")
    public String boxListPage(@ModelAttribute("search")BoxVO search, HttpServletRequest request, ModelMap model)  throws Exception {
        webStart(request);
        //---------------------------------------------------------------------
        // paging set
        //---------------------------------------------------------------------
        ...
        RtnVO rtn = boxService.searchList(search);
        RtnVO rtnTotCnt = boxService.searchListTotCnt(search);
        rtn.setTotCnt((Integer)rtnTotCnt.getObj());
        wedEnd(request, rtn, model);
        return "/basicinfo/boxList";
    }
    
    @RequestMapping(value = "/boxDtlPage.do")
    public String boxDtlPage(@ModelAttribute("search")BoxVO search, HttpServletRequest request, ModelMap model)  throws Exception {
        webStart(request);
        RtnVO rtn = null;
        search.setFactoryCd(proPertyService.getFactoryCd());
        if ("R".equals(search.getCrudType())) {
            rtn = boxService.select(search);
        } else {
            rtn = new RtnVO();
            rtn.setObj(new BoxVO());
        }
        
        wedEnd(request, rtn, model);
        return "/basicinfo/boxDtl";
    }
    
    @RequestMapping(value = "/boxSaveAct.do")
    public ModelAndView boxSaveAct(@ModelAttribute("obj")BoxVO obj, HttpServletRequest request, ModelMap model)  throws Exception {
        webStart(request);
        ...
        if ("C".equals(obj.getCrudType())) {
        	rtn = boxService.select(obj);
        	if(rtn.getObj() == null) {
        		rtn = boxService.insert(obj);
        	} else {
        	    BoxVO box = (BoxVO)rtn.getObj();
        	    if ("N".equals(box.getUseYn())) {
        	        obj.setUseYn("Y");
        	        rtn = boxService.update(obj);
        	    } else {
